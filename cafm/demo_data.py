...
def seed_action_dialog_demo_data():
    """Seed linked records for the CAFM related-record action dialogs."""
    from frappe.utils import add_months, add_years, nowdate
    ...
    provider = "CoolAir Services Demo"
    category = "HVAC"
    work_order_name = frappe.db.get_value(
        "Facility Work Order",
        {
    ...
    if not work_order_name:
        frappe.throw("An open HVAC demo Work Order with an Asset is required.")

    work_order = frappe.get_doc("Facility Work Order", work_order_name)
    contract_name = "CAFM Action Dialog HVAC Contract"
    if frappe.db.exists("Facility Service Contract", contract_name):
        contract = frappe.get_doc("Facility Service Contract", contract_name)
    else:
        contract = frappe.new_doc("Facility Service Contract")
        contract.contract_name = contract_name
    contract.service_provider = provider
    contract.company = company
    contract.contract_status = "Active"
    contract.start_date = add_months(nowdate(), -1)
    contract.end_date = add_years(nowdate(), 1)
    contract.currency = (
        frappe.db.get_value("Company", company, "default_currency") or "USD"
    )
    contract.contract_value = 18000
    contract.scope_type = "Service Category"
    contract.service_category = category
    contract.scope_notes = (
        "Demo contract used by the in-page matching-contracts window."
    )
    contract.save(ignore_permissions=True)

    quotation_specs = (
        ("CAFM Action Dialog - Standard Service", 1250, 137.5, 5),
        ("CAFM Action Dialog - Priority Service", 1680, 184.8, 2),
    )
    quotations = []
    for quotation_name, amount, tax, completion_days in quotation_specs:
        quotation_id = frappe.db.get_value(
            "Facility Vendor Quotation",
            {"quotation_name": quotation_name, "work_order": work_order.name},
            "name",
        )
        quotation = (
            frappe.get_doc("Facility Vendor Quotation", quotation_id)
            if quotation_id
            else frappe.new_doc("Facility Vendor Quotation")
        )
        quotation.quotation_name = quotation_name
        quotation.work_order = work_order.name
        quotation.service_provider = provider
        quotation.company = company
        quotation.quotation_status = "Received"
        quotation.scope_of_work = (
            work_order.work_description or work_order.subject
        )
        quotation.expected_completion_days = completion_days
        quotation.valid_until = add_months(nowdate(), 3)
        quotation.warranty_offered = "90-day workmanship warranty"
        quotation.service_contract = contract.name
        quotation.currency = contract.currency
        quotation.quoted_amount = amount
...
def verify_action_dialog_demo_data(user="Administrator"):
    """Return the same permission-aware records consumed by the three dialogs."""
    from cafm.api import get_cafm_doctype_detail
    from cafm.cafm.doctype.facility_service_contract.facility_service_contract import (
        get_matching_service_contracts,
    )

    seeded = seed_action_dialog_demo_data()
    frappe.set_user(user)
    open_work_orders = frappe.get_all(
        "Facility Work Order",
        filters={
            "asset": seeded["asset"],
            "work_order_status": ["not in", ["Closed", "Cancelled"]],
        },
        fields=["name", "subject", "work_order_status"],
        order_by="planned_end asc",
    )
    quotations = frappe.get_all(
        "Facility Vendor Quotation",
        filters={"work_order": seeded["work_order"]},
        fields=["name", "quotation_name", "quotation_status", "total_amount"],
        order_by="creation desc",
    )
    contracts = get_matching_service_contracts(seeded["work_order"])
    asset_actions = {
        action["id"]
        for action in get_cafm_doctype_detail(
            "Asset", seeded["asset"]
        )["actions"]
    }
    work_order_actions = {
        action["id"]
        for action in get_cafm_doctype_detail(
            "Facility Work Order", seeded["work_order"]
        )["actions"]
    }
    required_asset_actions = {"view_open_work_orders"}
    required_work_order_actions = {
        "view_vendor_quotations",
        "view_matching_contracts",
    }
    if not required_asset_actions.issubset(asset_actions):
        frappe.throw("The Asset open-work-orders action is unavailable.")
    if not required_work_order_actions.issubset(work_order_actions):
        frappe.throw("The Work Order quotation or contract action is unavailable.")
    if seeded["work_order"] not in {row.name for row in open_work_orders}:
        frappe.throw("The demo Work Order is missing from the Asset action list.")
    if not set(seeded["quotations"]).issubset({row.name for row in quotations}):
        frappe.throw("The demo quotations are missing from the Work Order list.")
    if seeded["contract"] not in {row.name for row in contracts}:
        frappe.throw("The demo contract does not match the Work Order.")
    ...
    return {
        **seeded,
        ...
        "matching_contract_count": len(contracts),
        "asset_actions": sorted(asset_actions),
        "work_order_actions": sorted(work_order_actions),
    }
